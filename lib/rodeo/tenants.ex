defmodule Rodeo.Tenants do
  ...
  def subscribe_organizations(%Scope{} = scope) do
    ...
    Phoenix.PubSub.subscribe(Rodeo.PubSub, "user:#{key}:organizations")
  end

  defp broadcast_organization(%Scope{} = scope, message) do
    key = scope.user.id

    Phoenix.PubSub.broadcast(Rodeo.PubSub, "user:#{key}:organizations", message)
  end
