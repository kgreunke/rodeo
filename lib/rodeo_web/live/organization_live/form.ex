defmodule RodeoWeb.OrganizationLive.Form do
  ...
  defp apply_action(socket, :edit, %{"id" => id}) do
    ...
    |> assign(:form, to_form(Tenants.change_organization(socket.assigns.current_scope, organization)))
  end
  ...
  def handle_event("validate", %{"organization" => organization_params}, socket) do
    changeset = Tenants.change_organization(socket.assigns.current_scope, socket.assigns.organization, organization_params)
  ...
  defp save_organization(socket, :edit, organization_params) do
    case Tenants.update_organization(socket.assigns.current_scope, socket.assigns.organization, organization_params) do
      {:ok, organization} ->
        {:noreply,
         socket
  ...
  defp save_organization(socket, :new, organization_params) do
    case Tenants.create_organization(socket.assigns.current_scope, organization_params) do
      {:ok, organization} ->
        {:noreply,
         socket
